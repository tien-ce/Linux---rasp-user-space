...
# 37 "main.c"
void modbusBuildRequest03PDU(ModbusMaster *status, uint16_t reg_addr, uint16_t count) { modbusBuildRequest01020304(status, 0x03, reg_addr, count); }
void modbusBuildRequest03RTU(ModbusMaster *status, uint8_t slave_addr, uint16_t reg_addr, uint16_t count) { status->index = 0; status->buffer[status->index++] = slave_addr; modbusBuildRequest03PDU(status, reg_addr, count); printf("Built RTU frame for Function %02X\n", 0x03); }
